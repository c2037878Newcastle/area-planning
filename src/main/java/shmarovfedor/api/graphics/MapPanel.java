...
public class MapPanel extends JPanel {
    ...
    public void drawPoints(Graphics g) {
        ...
        for (int i = 0; i < point.size(); i++)
            g.fillOval((int) (scale * point.get(i).getX() + leftBound) - 5,
    ...
    public void drawBuildings(Graphics g) {
        ...
        buildings.forEach(building -> {
            ...
            g.fillRect((int) (scale * (building.x() - building.type().width() / 2) + leftBound),
                    (int) (bottomBound - scale * (building.y() + building.type().height() / 2)),
                    (int) (scale * building.type().width()),
                    (int) (scale * building.type().height())
            );
            g.setColor(BUILDING_COLOR);
            g.drawRect(
                    (int) (scale * (building.x() - building.type().width() / 2) + leftBound),
                    (int) (bottomBound - scale * (building.y() + building.type().height() / 2)),
                    (int) (scale * building.type().width()),
                    (int) (scale * building.type().height())
            );
